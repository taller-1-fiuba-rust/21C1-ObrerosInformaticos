    ...
    pub fn handle(&self, request: &Request) -> Response {
        ...
        if method == *"GET".to_string() {
            ...
            match file_content {
                ...
                Err(_) => Response::new()
                    .with_status(404)
                    .with_body("Not found".to_string().as_bytes().to_vec()),
            }
            ...
            if request.endpoint() == "/eval" && valid_command(request.body().clone()) {
                ...
                match response {
                    Ok(resp) => Response::new().with_status(200).with_body(resp.to_vec()),
                    Err(_) => Response::new()
                        .with_status(404)
                        .with_body("fail to get response".as_bytes().to_vec()),
                }
            } else {
                Response::new()
                    .with_status(404)
                    .with_body("Request not correct".as_bytes().to_vec())
            }
        } else {
            Response::new()
                .with_status(404)
...
pub fn valid_command(body: String) -> bool {
    ...
    let cmd: String = body.split_whitespace().next().unwrap_or("").to_string();
