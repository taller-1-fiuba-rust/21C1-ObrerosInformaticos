...
pub fn run(pubsub: Arc<Mutex<PublisherSubscriber>>, builder: &mut ResponseBuilder, arguments: Vec<ProtocolType>) -> Result<(), String> {
    ...
    let channel = match arguments[0].clone().string() {
        Ok(s) => s,
        Err(s) => {
            return Err(format!("Error '{}' while parsing channel'{}'", s, arguments[0].to_string()));
        }
    };
    ...
    let msg = match arguments[1].clone().string() {
      Ok(s) => s,
      Err(s) => {
          return Err(format!("Error '{}' while parsing message'{}'", s, arguments[1].to_string()));
      }
    };
    ...
    builder.add(ProtocolType::Integer(locked_pubsub.publish( channel, msg) as i32));
    Ok(())
}